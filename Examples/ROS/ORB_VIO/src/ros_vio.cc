...
int main(int argc, char **argv)
{
    ...
    //std::string path_to_vocabulary = "/home/shinan/Project/LearnVIORB/Vocabulary/ORBvoc.bin";
    //std::string path_to_settings = "/home/shinan/Project/LearnVIORB/config/rs_rgb_imu_official.yaml";
     ...
     */
    double imageMsgDelaySec = config.GetImageDelayToIMU(); //0.0280180128084
    ...
    BOOST_FOREACH(rosbag::MessageInstance const m, view)
    {
        ...
        if(bdata)
        {//将IMU数据转换格式，每一张图像对应其前面的10组imu数据
            ...
            for(unsigned int i=0;i<vimuMsg.size();i++)
            {
                ...
                /*if (i==0)
                    std::cout<<"First IMU data: "<<std::setprecision(12)<<imuMsg->header.stamp.toSec();
                if(i==vimuMsg.size()-1)
                    std::cout<< "Last IMU data: " <<std::setprecision(12) <<imuMsg->header.stamp.toSec();*/
            ...
            cv::Mat im = cv_ptr->image.clone();
            {
                ...
                //std::cout<<"Image TimeStamp: "<<std::setprecision(12)<< imageMsg->header.stamp.toSec()<<std::endl;
            ...
            bool bstop = false;
            /*while(!SLAM.bLocaMessageInstancelMapAcceptKF())
            {
                if(!ros::ok())
                {
                    bstop=true;
